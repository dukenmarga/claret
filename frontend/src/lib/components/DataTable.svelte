<script>
    ...
    // Nested view mode: 'collections' or 'data'
    let nestedViewMode = $state({}) // path -> 'collections' | 'data'
                        ...
                        <tr class="bg-slate-900/60 shadow-inner">
                            <td colspan="100" class="p-0 border-l-2 border-indigo-500/50">
                                <div class="pl-12 pr-4 py-3 space-y-4">
                                    <!-- Tabs for Data vs Collections -->
                                    <div class="flex items-center gap-4 border-b border-slate-800/50 pb-2">
                                        <button 
                                            onclick={() => nestedViewMode[doc.path] = 'collections'}
                                            class="text-[10px] uppercase font-bold tracking-widest flex items-center gap-1.5 transition-colors {!nestedViewMode[doc.path] || nestedViewMode[doc.path] === 'collections' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}"
                                        >
                                            <Folder size={12} /> Sub-collections
                                        </button>
                                        <button 
                                            onclick={() => nestedViewMode[doc.path] = 'data'}
                                            class="text-[10px] uppercase font-bold tracking-widest flex items-center gap-1.5 transition-colors {nestedViewMode[doc.path] === 'data' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}"
                                        >
                                            <Braces size={12} /> Data Tree
                                        </button>
                                    </div>
                                            ...
                                            {#if appState.docSubCollections[doc.path]}
                                                {#each appState.docSubCollections[doc.path] as subColl}
                                                    ...
                                                    <div class="flex flex-col">
                                                        <button 
                                                            onclick={() => appState.toggleCollectionExpansion(collPath)}
                                                            class="flex items-center gap-2 text-xs text-indigo-400 hover:text-indigo-300 transition-colors py-1 group/coll w-fit"
                                                        >
                                                            ...
                                                            <div class="pl-6 border-l border-slate-800 mt-1 mb-2 space-y-1">
                                                                {#if appState.collectionDocuments[collPath]}
                                                                    {#each appState.collectionDocuments[collPath] as nestedDoc}
                                                                        <button 
                                                                            onclick={() => appState.selectedDoc = nestedDoc}
                                                                            class="flex items-center gap-2 group/nested text-slate-400 hover:text-white py-1 px-2 rounded-md hover:bg-slate-800/40 w-full text-left transition-all"
                                                                        >
                                                                            <FileText size={10} class="text-slate-600" />
                                                                            <span class="text-[11px] font-mono flex-1">{nestedDoc.id}</span>
                                                                            <span class="text-[9px] text-slate-600 opacity-0 group-hover/nested:opacity-100 uppercase font-bold tracking-tighter">Click to inspect</span>
                                                                        </button>
                                                                    {/each}
                                                                    {#if appState.collectionDocuments[collPath].length === 0}
                                                                        <div class="text-[10px] text-slate-600 italic px-2">Empty collection</div>
                                                                    {/if}
                                                                {:else}
                                                                    <div class="text-[10px] text-slate-600 animate-pulse px-2">Fetching documents...</div>
                                                                {/if}
                                                            </div>
                                                        {/if}
                                                    </div>
                                                ...
                                                {#if appState.docSubCollections[doc.path].length === 0}
                                                    <div class="text-[10px] text-slate-600 italic">No sub-collections found</div>
                                                {/if}
                                            {:else}
                                                <div class="text-[10px] text-slate-600 animate-pulse">Loading...</div>
                                            {/if}
                                        </div>
                                    {:else}
                                        <!-- Data Tree View -->
                                        <div class="bg-slate-950/40 rounded-lg p-4 font-mono text-[11px] border border-slate-800/50">
                                            {#each Object.entries(doc.data) as [key, value]}
                                                <div class="flex gap-2 py-0.5 group/field">
                                                    <span class="text-indigo-400 font-bold w-32 shrink-0 truncate">{key}</span>
                                                    <span class="text-slate-500 mr-1">:</span>
                                                    <span class="text-emerald-400 break-all">{formatValue(value)}</span>
                                                </div>
                                            {/each}
                                            <div class="mt-4 pt-2 border-t border-slate-800/50 flex justify-end">
                                                <button onclick={() => appState.selectedDoc = doc} class="text-[9px] text-indigo-500 uppercase font-bold hover:text-indigo-400">Open in Full Inspector</button>
                                            </div>
                                        </div>
                                    {/if}
                                </div>
...
{#if showDuplicateDialog}
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4 w-full max-w-sm shadow-2xl">
            ...
            <input bind:value={targetCollection} type="text" placeholder="Target Path (e.g. backup_users)..." class="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500" />
